...
public class XfileClass {
	...
	public synchronized int sys_fseek(int type, int moveDis) {
		...
		seekIndex = oldIndex;
		if (type == SYS_FST_CURRENT_DOWN) {// �ӵ�ǰλ�����¶�λ
			try {
				seekIndex += moveDis;
		...
		} else if (type == SYS_FST_CURRENT_UP) {// �ӵ�ǰλ�����϶�λ
			seekIndex -= moveDis;
			if (seekIndex < 0) {
				seekIndex = 0;
			}
		} else if (type == SYS_FST_START) {// ��ͷ��ʼ��λ
			seekIndex = moveDis;
		} else if (type == SYS_FST_END) {// ��β����ʼ��λ
			try {
				seekIndex = (int) mRandomFile.length();
				// ע������Ϊ3.x�еĴ��?ʽ
			} catch (IOException e) {
		...
		return seekIndex;// 3.x����ΪoldIndex��Ӧ�÷���ΪseekIndex
	}
